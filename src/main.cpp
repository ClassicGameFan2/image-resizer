...
int main(int argc, char** argv) {
    ...
    std::string algo = "fsr";
    bool useRcas = true; // RCAS is ON by default
    ...
    if (inputFile.empty() || outputFile.empty()) {
        std::cout << "Usage: image-resizer.exe <in.png> <out.png> [--scale 2.0] [--algo fsr|nearest] [--rcas on|off] [--sharpness 0.2]" << std::endl;
        return 1;
    }
    ...
    if (algo == "nearest") {
        ...
        std::cout << "Processing (AMD FSR 1.0 EASU)..." << std::endl;
        
        if (useRcas) {
            unsigned char* easuData = new unsigned char[newW * newH * 4];
            scaleFSR_EASU(imgData, width, height, easuData, newW, newH);
            
            std::cout << "Processing (AMD FSR 1.0 RCAS @ " << sharpness << " sharpness)..." << std::endl;
            applyFSR_RCAS(easuData, newW, newH, finalData, sharpness);
            delete[] easuData;
        } else {
            std::cout << "RCAS Skipped. Outputting pure EASU." << std::endl;
